                ...
                for(int i = 0; i < dynamic_obj->object_count; ++i) {
                        switch(root_node->type) {
                                case BALL: {
                                                   ...
                                                   ball->circ->move_position(ball->velocity*(1.0/60.0)); 
                                           } break;
                        }
...
GAME_INIT_FUNC(init) {
        ...
        init_grid(&gamestate->overlord, gamestate->window_width_height.x, gamestate->window_width_height.y, 5, 5 , vec2(0, 0));
